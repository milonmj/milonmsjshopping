  ...
  return (
    <div className="mx-auto max-w-md px-4 py-16">
      ...
      <p className="mt-1 text-sm text-brand-ink/60">
        {locale === "bn" ? "অর্ডার ট্র্যাক করুন ও দ্রুত চেকআউটের জন্য" : "Track orders and check out faster next time"}
      </p>

      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        <input required placeholder={locale === "bn" ? "পূর্ণ নাম" : "Full Name"} value={form.name}
          onChange={(e) => setForm({ ...form, name: e.target.value })}
          className="w-full rounded-lg border border-brand-pink/40 px-4 py-2.5 text-sm" />

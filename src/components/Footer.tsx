...
export default function Footer({ locale = "bn", categories = [] }: { locale?: "bn" | "en"; categories?: NavCategory[] }) {
  const t = locale === "bn"
    ? {
        about: "মিলন এম অ্যান্ড জে শপিং – পোশাক, কসমেটিক্স, জুতা ও ইলেকট্রনিক পণ্যের বিশ্বস্ত অনলাইন শপ।",
  ...
  const policyLinks = [
    { slug: "about", nameBn: "আমাদের সম্পর্কে", nameEn: "About Us" },
    { slug: "contact", nameBn: "যোগাযোগ", nameEn: "Contact" },
    { slug: "faq", nameBn: "সচরাচর জিজ্ঞাসা", nameEn: "FAQ" },
    { slug: "privacy-policy", nameBn: "প্রাইভেসি পলিসি", nameEn: "Privacy Policy" },
    { slug: "terms", nameBn: "শর্তাবলী", nameEn: "Terms & Conditions" },
    { slug: "return-policy", nameBn: "রিটার্ন পলিসি", nameEn: "Return Policy" },
  ];
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:grid-cols-2 lg:grid-cols-5">
        <div>
          <div className="mb-3 flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-pink text-sm font-bold text-white">M</span>
        ...
        <div>
          <h4 className="mb-3 font-semibold text-white">{locale === "bn" ? "নীতিমালা" : "Policies"}</h4>
          <ul className="space-y-2 text-sm">
            {policyLinks.map((p) => (
              <li key={p.slug}><Link href={`/${p.slug}`}>{locale === "bn" ? p.nameBn : p.nameEn}</Link></li>
            ))}
          </ul>
        </div>
      </div>
